...
if [[ "$FULL" -eq 1 ]]; then
  ...
  "$PYTHON" -m pytest tests/test_post_validation.py tests/test_blog_converter.py tests/test_episodeseries_registry.py tests/test_list_pages.py::TestListTemplateStructure -q
fi
